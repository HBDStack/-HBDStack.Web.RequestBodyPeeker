...
{
    public class RequestPeekWithEncodingTests
    {
        ...
        public void Peek_TestToIfTheStreamIsReadableAgain()
        {
            ...
            var peekBody = _httpContext.Request.PeekBody(new UTF8Encoding());
        ...
        public async Task PeekAsync_TestToIfTheStreamIsReadableAgain()
        {
            const string body = "{ali:alp}";
            _httpContext = MakeFakeContext(body);
            var peekBody = await _httpContext.Request.PeekBodyAsync(new UTF8Encoding());
            ...
            var contentLen = (int) _httpContext.Request.ContentLength;
            var buffer = new byte[contentLen];
            var readAsync = await _httpContext.Request.Body.ReadAsync(buffer, 0, contentLen);
        ...
        public void PeekT_TestToIfTheStreamIsReadableAgain()
        {
            ...
            var peekBody = _httpContext.Request.PeekBody<DummyClass>(new UTF8Encoding());
            ...
            var contentLen = (int) _httpContext.Request.ContentLength;
            var buffer = new byte[contentLen];
            var read = _httpContext.Request.Body.Read(buffer, 0, contentLen);
        ...
        public async Task PeekTAsync_TestToIfTheStreamIsReadableAgain()
        {
            ...
            _httpContext = MakeFakeContext(JsonConvert.SerializeObject(myClass));
            var peekBody = await _httpContext.Request.PeekBodyAsync<DummyClass>(new UTF8Encoding());
